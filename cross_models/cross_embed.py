...
def dsw(seg_len, linear, x, x2 = None):
    ...
    if x2 is not None:
        ...
        x2_segment = x2.repeat_interleave(
            ts_len//seg_len*ts_dim//x2.shape[1], dim=1).reshape(-1,1)
        x_segment = torch.cat((x_segment, x2_segment), axis=1)
    x_embed = linear(x_segment)
    return rearrange(
        x_embed, '(b d seg_num) d_model -> b d seg_num d_model', 
        b = batch, d = ts_dim)
