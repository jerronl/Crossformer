...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, opt, path):
        score = -val_loss
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, opt, path)
    ...
    def save_checkpoint(self, val_loss, model, opt, path):
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        torch.save((model.state_dict(), opt.state_dict()), path+'/'+'checkpoint.pth')
